...
class Config:
    ...
    def validate(cls) -> None:
        ...
        # Note: We allow absolute paths for Docker deployments
        try:
        ...
        # Only validate for path traversal if not absolute (Docker uses absolute paths)
        if not os.path.isabs(real_chromadb):
            if '..' in real_chromadb:
                raise ValueError("CHROMADB_PATH cannot contain '..' traversal sequences")
        if not os.path.isabs(real_adoc):
            if '..' in real_adoc:
                raise ValueError("ADOC_FILES_PATH cannot contain '..' traversal sequences")
